...
export class SwordPvpTwo extends EventEmitter {
  ...
  async logHealth(health: number) {
    ...
    if (!isNaN(health))
      console.log(`Dealt ${newHealth} damage. Target ${this.target?.username} has ${health} health left.`);
  }
    ...
    switch (this.options.critConfig.mode) {
      ...
      case "hop":
        ...
        if (this.timeToNextAttack === 7 || this.firstHit) {
          ...
          if (this.botReach() <= this.options.genericConfig.attackRange + 2) {
            const roughGroundCheck = this.target.position.y.toFixed(5).split(".")[1].charAt(4) === "0";
            if (roughGroundCheck && this.botReach() > this.options.genericConfig.attackRange) return false;
            this.bot.setControlState("jump", true);
            this.bot.setControlState("jump", false);
            this.reactionaryCrit();
          } else {
  ...
  async doMove() {
    ...
    if (farAway) {
      ...
      const shouldApproach = this.botReach() > this.options.genericConfig.tooCloseRange; //this.options.genericConfig.attackRange / 20;
